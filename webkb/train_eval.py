...
def evaluate(model, data):
    ...
    for key in ['train', 'val', 'test']:
        idx = data['{}_idx'.format(key)]
        loss = F.nll_loss(logits[idx], data['{}_y'.format(key)]).item()

        outs['{}_loss'.format(key)] = loss
        outs['{}_acc'.format(key)] = f1_score(data['{}_y'.format(key)].cpu(), logits[idx].max(1)[1].cpu(), average='micro')
        outs['{}_macro_f1'.format(key)] = f1_score(data['{}_y'.format(key)].cpu(), logits[idx].max(1)[1].cpu(), average='macro')
